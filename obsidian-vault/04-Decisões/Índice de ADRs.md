...
| 0163 | [Padrão único de sinal de espera para ação disparada pelo operador](../../docs/decisions/0163-padrao-de-espera-em-dialogos.md) — `ProgressoIndeterminado` unifica 20 pontos (criar/adicionar-variação, criar-kit, kit-virtual, Publicar no ML, os 6 de `Publicados`, saque, entrada, ajuste, pulse adicionar/reprecificar, exportar, 3 remoções de foto): glow+barra em ação normal, só barra em destrutiva. Recorte de sobriedade em 2026-09-17 removeu 16 pontos (9 de espera curta local, 1 código morto, 1 classificação errada, 5 botões fora de modal) — o modo "fora de modal" saiu do padrão inteiro. Critério final: só se a operação fala com Mercado Livre/storage/fila/IA E o modal fica de pé. Grupo B segura o modal aberto até a resposta (mudança de comportamento, não só visual); `onOpenChange` não é guardado porque `chamarEdge` não tem timeout. Emenda ao `contrato-motion-v5.md` §9/§10. |
