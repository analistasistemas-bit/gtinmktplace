...
| 0163 | [Padrão único de sinal de espera para ação disparada pelo operador](../../docs/decisions/0163-padrao-de-espera-em-dialogos.md) — `ProgressoIndeterminado` unifica 21 modais + 6 confirmações de `Publicados` + saque + 3 remoções de foto + 5 pontos fora de modal: glow+barra em ação normal, só barra em destrutiva e fora de modal. Grupo B passa a segurar o modal aberto até a resposta (mudança de comportamento, não só visual); destrutivo é definido pelo efeito real no Mercado Livre, não pela palavra do botão; `onOpenChange` não é guardado porque `chamarEdge` não tem timeout. Emenda ao `contrato-motion-v5.md` §9/§10. |
