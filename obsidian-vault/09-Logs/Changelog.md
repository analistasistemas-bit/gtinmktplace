...
- **Auditoria `improve` nova (rodada 3) na superfície de mensagens pós-venda (ADR-0067/0068)** — sem
  IDOR cross-tenant (classe do plano 017 não regrediu). Planos 034-038 escritos; 034-037 executados
  e mergeados nesta sessão:
  - **034 — testes de caracterização** (`upsertMensagens`, `buscarConversas`, `responderMensagemPedido`)
    antes de mexer no fluxo. +12 testes.
  - **035 — fix: follow-ups de mensagens nunca sincronizavam em tempo real.** O `ml-webhook` deduplica
    por `(topic, resource)`, mas o resource de `messages` é o mesmo para toda a conversa — da 2ª
    mensagem em diante o insert de dedup conflitava e o webhook nunca reenfileirava (só o backfill
    horário recuperava, sem alertar). `sync-mensagem` agora **apaga** a linha de dedup ao processar
    (reabre para a próxima mensagem); o webhook reenfileira se a linha ficar órfã (>2min, nunca
    processada — job perdido). Alerta migrado de chat único da org (`lerConfigTelegram`) para
    `notificarCategoria(..., 'mensagens', ...)`, categoria nova (ADR-0068). Deploy `ml-webhook`,
    `sync-mensagem`, `usuarios` (a última também consome a lista de categorias — esquecida no 1º
    deploy, corrigida no ato). Ver [[Problemas Resolvidos]].
  - **036 — perf: badge "aguardando resposta" via RPC server-side.** Antes baixava a tabela
    `ml_mensagens` inteira do usuário a cada navegação para agregar no browser. RPC
    `contar_conversas_aguardando()` conta packs cuja última mensagem é do comprador — mesmo padrão
    de `usePerguntasNaoRespondidas`, com agregação por pack. `buscarConversas` (lista da aba) ganhou
    `.limit(1000)` defensivo — corrigido para desc+reverse (asc+limit pegaria as mensagens mais
    ANTIGAS, não as recentes).
  - **037 — hardening (5 itens independentes):** (A) revogado `grant all` de `anon` em
    `ml_mensagens` (PII de comprador, contradizia o próprio comentário da migration); (B) valida
    `pack_id` (`/^\d+$/`) em `responder-mensagem` antes de entrar numa query `.or()` do PostgREST;
    (C) mensagem sem `data_ml` não decide mais o badge (nulls vão para o início cronológico, nunca
    para o fim); (D) `upsertMensagens` conta "novas" a partir do retorno do próprio upsert
    (`ignoreDuplicates` + `select`), eliminando a race de contagem dupla entre execuções
    concorrentes; (E) `usuarios` bloqueia um admin comum alterando um super-admin da mesma org.
- **Liveness da integração ML (ADR-0069) — spike 032 → ADR → implementação, planos 038-040.**
  Motivação: um token ML revogado produzia a MESMA tela que um dia sem vendas — `sync-venda`,
  `sync-pergunta`, `sync-devolucao` engoliam qualquer falha (`catch { semCredencial: true }, 200`)
  sem alertar ninguém; já causou incidente 2× antes (TypeError engolido 11 dias; `verify_jwt`
  errado sem alerta). Diego decidiu as 4 questões em aberto do spike (estado por CONEXÃO, alerta
  na 1ª falha permanente-auth, reset automático, categoria `integracao` nova):
  - **038 — ADR-0069** escrito com as decisões; gerou o plano 039.
  - **039 — fase 1**: `sync-venda`/`sync-pergunta`/`sync-devolucao`/`reconciliar-faturamento`
    classificam o erro (401/403 = conexão morta, grava `marketplace_connections.auth_alerta_em` +
    alerta Telegram categoria `integracao` só na 1ª falha; 404 = recurso não existe; resto =
    transiente, QStash re-tenta). Sucesso grava `ultima_sincronizacao_ok_em` e reseta o alerta.
    `responder-pergunta` ganhou um `try/catch` pra não deixar a mudança de assinatura de
    `buscarPergunta` (null→throw) virar erro num re-fetch best-effort que roda DEPOIS da resposta
    já ter sido enviada ao comprador — achado do executor, não estava no spike original. Migrations
    aplicadas, 5 funções deployadas.
  - **040 — fecha um gap que o próprio 039 encontrou**: a renovação de token (`POST /oauth/token`)
    responde **400** (não 401) quando o `refresh_token` é revogado (ADR-0012 já documentava isso),
    e caía em "transiente" sem alertar. Fix cirúrgico: extrai o campo OAuth2 `error` do corpo e só
    trata `invalid_grant` como conexão morta — sem generalizar pra qualquer 400 (evita alarme falso
    no cenário de corrida de refresh concorrente, também do ADR-0012). 4 funções redeployadas.
  - **Pendência aceita**: o formato real do erro `invalid_grant` do ML nunca foi observado ao vivo
    (só documentado pela RFC 6749) — pior caso, `oauthError` fica `null` e o comportamento é
    idêntico ao pré-fix. Confirma organicamente na 1ª revogação real de produção.
