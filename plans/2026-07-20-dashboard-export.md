...
2. Acrescentar um teste com `incluirKpis: false` e coleções vazias, afirmando
   `kpis === undefined`, zero linhas e ausência de blocos vazios.
...
5. Em `src/pages/Dashboard.tsx`, importar `BotaoExportar` e
   ...
   diretamente ao adaptador. Não criar consulta, estado ou componente novo.
6. Executar
   `rtk pnpm vitest run tests/lib/export/adapters.test.ts tests/pages/Dashboard.test.tsx`;
   todos os testes direcionados devem passar.
7. Executar `rtk tsc -b --pretty false`; resultado esperado: zero erros.
8. Executar
   `rtk eslint src/pages/Dashboard.tsx src/lib/export/adapters.ts tests/pages/Dashboard.test.tsx tests/lib/export/adapters.test.ts`;
   resultado esperado: zero erros.
9. Confirmar por inspeção que o controle permanece um botão nativo, com nome
   acessível visível, operação por teclado fornecida pelo componente existente
   e sem perda de foco no diálogo.
10. Criar um commit único com implementação, testes e este plano.
...
- Cobertura do design: botão no cabeçalho, período, canal, KPIs, evolução, top
  ...
  formatos são fornecidos pelo `BotaoExportar` existente.
...
- Consistência de tipos: todos os argumentos são tipos existentes; a página já
  ...
  síncrono `ReportData`.
- Estado testável e commitável: a única tarefa termina com testes direcionados,
  typecheck, lint direcionado, inspeção de acessibilidade e commit único.
