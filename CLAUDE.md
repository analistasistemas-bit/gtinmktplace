...
**Status do projeto:** M0+M1+M2+M3+M3.1 ✅ concluídos; M4 (Integração ML) em andamento — **OAuth ✅** + **concorrência ✅** + **estratégia de preço ✅** + **categorias/atributos ✅** + **potencial de venda ✅** (process-familia v18) + **pré-condição ADR-0013 ✅** (edge cases da planilha não-bloqueantes; deploy MCP do ingest-lote pendente); próximo bloco do M4: **publicação CREATE/UPDATE** (inclui seleção granular do que publicar — pedido do Diego)
