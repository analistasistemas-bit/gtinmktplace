...
**Status do projeto:** M0+M1+M2+M3+M3.1 ✅ concluídos; M4 (Integração ML) em andamento — **OAuth ✅** + **concorrência ✅** + **estratégia de preço ✅** + **categorias/atributos ✅** + **potencial de venda ✅** (process-familia v18) + **pré-condição ADR-0013 ✅** + **Publicação CREATE implementada** (plano-10: seleção do que publicar na Revisão + edges `publicar-familias` v1 / `publish-familia-ml` v2; **falta bug bash com token real** — GTIN/listing_type/foto); próximo: bug bash da Publicação CREATE, depois **Publicação UPDATE** (ADR-0005)
