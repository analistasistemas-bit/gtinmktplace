...
**Status do projeto:** M0+M1+M2+M3+M3.1 ✅ concluídos; M4 (Integração ML) em andamento — **bloco OAuth ✅** + **busca de concorrência (código + integração) ✅** (Tasks 8+9 do plano-07; falta bug bash Task 10 com token real); próximos blocos do M4: estratégia de preço, categorias, publicação CREATE/UPDATE
...
| 2026-06-01 | M4 busca de concorrência Tasks 8+9 ✅: migration `add_concorrencia_familias` aplicada via MCP (2 enums `origem_concorrencia`/`classe_concorrencia` + 4 colunas `concorrencia_*` em `familias`), tipos regenerados. `process-familia` **v14** deployada via MCP integra `buscarConcorrencia` 1×/família após a copy (resiliente: erro → `nenhuma`/PRÓPRIO seguro, ADR-0014); inclui `gtin` no select das variações e persiste os 4 campos. Deploy re-empacotou os shared novos (`_shared/ml/{concorrencia,token,refresh-decisao}.ts`, `_shared/concorrencia/*`, `_shared/redis/cache-concorrencia.ts`) + `redis/client.ts` atualizado (`redisSetNX`). 122/122 testes verdes, lint verde. Falta Task 10 (bug bash com token ML real — `ml_credentials` vazio pós-disconnect do bug bash OAuth, reconectar antes). | 
