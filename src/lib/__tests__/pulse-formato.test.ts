...
const vinculado = (
  ...
  ptw_aplicavel: boolean | null = null,
  ptw_preco_sugerido: number | null = null,
) => ({ ptw_status, catalogo_status: 'vinculado', origem: 'auto', ptw_aplicavel, ptw_preco_sugerido }) as const;
...
describe('seloPriceToWin — applicable_suggestion', () => {
  it('referência marcada como não aplicável não vira posição de preço', () => {
    const selo = seloPriceToWin(vinculado('with_benchmark_high', false), null);
  ...
  it('aplicável mantém o selo normal', () => {
    expect(seloPriceToWin(vinculado('with_benchmark_high', true), null)?.texto).toBe('Acima da referência');
  });
  ...
  it('campo ausente (null) mantém o comportamento anterior', () => {
    expect(seloPriceToWin(vinculado('with_benchmark_high', null), null)?.texto).toBe('Acima da referência');
  });

  it('não aplicável sai da escala de ordenação de preço', () => {
    expect(ordemPriceToWin(vinculado('no_benchmark_lowest', false), null)).toBe(99);
    expect(ordemPriceToWin(vinculado('no_benchmark_lowest', true), null)).toBe(0);
  });
});
...
describe('seloPriceToWin', () => {
  ...
  it('traduz os quatro status de referência de preço do ML', () => {
    expect(seloPriceToWin(vinculado('no_benchmark_lowest'), null)?.texto).toBe('Abaixo da referência');
    expect(seloPriceToWin(vinculado('no_benchmark_ok'), null)?.texto).toBe('Na referência');
    expect(seloPriceToWin(vinculado('with_benchmark_high'), null)?.texto).toBe('Acima da referência');
    expect(seloPriceToWin(vinculado('with_benchmark_highest'), null)?.texto).toBe('Acima de todos');
  });

  // Não é dado auditável (não dá pra abrir e conferir, como dá com o menor concorrente real) — os
  // quatro status de posição ficam neutros, nunca verde/laranja/vermelho, mesmo o "abaixo" bom.
  it('os quatro status de posição de preço são sempre tom neutro', () => {
    for (const s of ['no_benchmark_lowest', 'no_benchmark_ok', 'with_benchmark_high', 'with_benchmark_highest']) {
      expect(seloPriceToWin(vinculado(s), null)?.tom).toBe('neutro');
    }
  });

  it('nenhum status descreve ausência de concorrentes', () => {
    for (const s of ['no_benchmark_lowest', 'no_benchmark_ok', 'with_benchmark_high', 'with_benchmark_highest']) {
      expect(seloPriceToWin(vinculado(s), null)?.texto).not.toMatch(/concorrência/i);
    }
  });

  it('estados de Markdown viram promoção, e ficam fora da escala de preço', () => {
    expect(seloPriceToWin(vinculado('promotion_active'), null)?.texto).toBe('Promoção ativa');
    expect(seloPriceToWin(vinculado('not_optin_applied'), null)?.texto).toBe('Promoção sugerida');
    // 99 = "sem posição na escala", não "o mais caro".
    expect(ordemPriceToWin(vinculado('promotion_active'), null)).toBe(99);
  });
  ...
  it('escala ordena do mais barato ao mais caro', () => {
    const escala = ['no_benchmark_lowest', 'no_benchmark_ok', 'with_benchmark_high', 'with_benchmark_highest']
      .map((s) => ordemPriceToWin(vinculado(s), null)!);
  ...
  it('status desconhecido não vaza jargão de API nem sugere posição de preço', () => {
    const s = seloPriceToWin(vinculado('status_novo_do_ml'), null);
    expect(s?.texto).toBe('Status não reconhecido');
    expect(s?.ajuda).toContain('status_novo_do_ml');
    expect(ordemPriceToWin(vinculado('status_novo_do_ml'), null)).toBe(99);
  });

  it('sem referência e sem vínculo: explica a causa (era um traço mudo)', () => {
    const s = seloPriceToWin(
      { ptw_status: null, catalogo_status: 'ficha_divergente', origem: 'auto', ptw_aplicavel: null, ptw_preco_sugerido: null },
      null,
    );
  ...
  it('ficha manual: a referência é sobre anúncio nosso, então não se aplica', () => {
    const s = seloPriceToWin(
      { ptw_status: null, catalogo_status: null, origem: 'manual', ptw_aplicavel: null, ptw_preco_sugerido: null },
      null,
    );
  ...
  it('vinculado mas ainda sem avaliação do ML: nada a dizer', () => {
    expect(seloPriceToWin(
      { ptw_status: null, catalogo_status: 'vinculado', origem: 'auto', ptw_aplicavel: null, ptw_preco_sugerido: null },
      null,
    )).toBeNull();
  });

  it('referência abaixo do menor concorrente real não vira selo — contradiria o dado auditável', () => {
    const selo = seloPriceToWin(vinculado('with_benchmark_high', true, 75.90), 109.99);
    expect(selo).toBeNull();
  });

  it('referência que não contradiz o menor concorrente real mantém o selo, com tom neutro', () => {
    const selo = seloPriceToWin(vinculado('with_benchmark_high', true, 120), 109.99);
    expect(selo?.texto).toBe('Acima da referência');
    expect(selo?.tom).toBe('neutro');
  });

  it('sem concorrente real conhecido, a referência não é suprimida', () => {
    expect(seloPriceToWin(vinculado('with_benchmark_high', true, 75.90), null)?.texto).toBe('Acima da referência');
  });
});
