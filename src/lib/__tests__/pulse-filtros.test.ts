...
describe('filtrarProdutos — situação no radar', () => {
  const lista = [produto({ id: 'a', status: 'ativo' }), produto({ id: 'b', status: 'pausado' })];

  it('separa ativos de pausados, e "todos" devolve os dois', () => {
    expect(filtrarProdutos(lista, filtros({ status: 'ativo' }), menorFixo(null)).map((p) => p.id)).toEqual(['a']);
    expect(filtrarProdutos(lista, filtros({ status: 'pausado' }), menorFixo(null)).map((p) => p.id)).toEqual(['b']);
    expect(filtrarProdutos(lista, filtros({ status: 'todos' }), menorFixo(null))).toHaveLength(2);
  });
});
...
describe('filtrarProdutos — combinação', () => {
  it('busca, situação e recorte se acumulam', () => {
    const lista = [
      produto({ id: 'alvo', titulo: 'Gel', status: 'ativo', meu_preco: 120 }),
      produto({ id: 'pausado', titulo: 'Gel', status: 'pausado', meu_preco: 120 }),
      produto({ id: 'outro', titulo: 'Solar', status: 'ativo', meu_preco: 120 }),
      produto({ id: 'barato', titulo: 'Gel', status: 'ativo', meu_preco: 80 }),
    ];
