...
- [ ] **UI não atualiza sozinha após "Reenviar" na Revisão** (achado 2026-07-22, durante validação do ADR-0088) — Diego reportou que a tela fica presa em "Ainda em processamento" e só reflete o status real (`pronto`/`publicando`/`erro`) depois de dar F5 manual. Provável falta de invalidação/realtime na mutation de reenviar (comparar com o padrão de invalidação já usado em outras mutations da Revisão). Investigar depois de confirmar a publicação da família de 9 cores (PAI 03103331).
...
**Fase 2, registrada como pendente (não bloqueia a entrega de hoje):**
- UPDATE por item filho + mini-saga de composição (adicionar/retirar cor de família UP já publicada).
- Reconciliador de convergência automatizado (hoje, se a ativação falhar parcial, só o "Reenviar" manual resolve — funciona porque a saga reaproveita itens já criados, mas não há convergência em background).
- Reconciliador de backfill (importar itens planos pré-existentes do ADR-0084/0087 pro modelo novo).
- **Vinculação de catálogo (ADR-0021, "concorrência") não roda no caminho UP** — achado durante a validação real: o caminho Legacy chama `enfileirarVinculacaoCatalogo` automaticamente, o caminho UP (9 itens separados) não tem equivalente ainda. Por ora, precisa ser feito manualmente por item ("Buscar" na tela de concorrência) até essa etapa ser construída.
- Guarda completa de remoção UP (mini-saga: pausar todos os N filhos + confirmar por GET + só então deletar local) — hoje só existe a guarda mínima que recusa a remoção.
