...
- [ ] **Camada 2 (próximo épico SaaS) — UI de atributos + categoria livre:** produto não-aviamento cujo obrigatório a IA não infere trava com aviso, mas o operador ainda não pode editar atributos nem escolher categoria arbitrária (`definir-categoria-familia` cobre só os 4 aviamentos). Precisa de brainstorming/design. **Dívida multi-tenant relacionada:** marca padrão `Avil` hard-coded em `atributos.ts` (`montarAtributosBase`/`montarAtributosML`) — trocar por config da empresa quando houver multi-tenant real.
