...
- [ ] **Camada 2B (fallback UI) — a planejar após 2A validado:** editor de atributos faltantes na Revisão (evoluir `card-categoria.tsx` do read-only "Faltam:" para editável), flag `atributos_editados_pelo_operador` + guarda em `process-familia`, edge function expondo o schema da categoria, trava de publicação enquanto houver faltante. Categoria livre = fase posterior. **Dívida multi-tenant:** marca padrão `Avil` hard-coded em `atributos.ts` — trocar por config da empresa no multi-tenant real.
...
- [x] **Notificação Telegram de liberação** — edge `notificar-liberacao` (pública/QStash, idempotente via coluna `ml_vendas.liberacao_notificada_em`): avisa quando o dinheiro das vendas é liberado HOJE em BRT no saldo Mercado Pago. Reusa a infra de Telegram. Migration `20260623160000_ml_vendas_liberacao_notificada.sql`.
  - **PENDENTE (Diego, infra):** (1) aplicar a migration; (2) deploy `supabase functions deploy notificar-liberacao --no-verify-jwt`; (3) smoke test; (4) **QStash schedule diário** (ex.: `0 12 * * *` UTC ≈ 09h BRT) → `.../functions/v1/notificar-liberacao`.
