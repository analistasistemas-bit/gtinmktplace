...
- [x] **Causa medida (15/09, sem login):** `https://www.mercadolivre.com.br/vendas/pacote/{id}/detalhe`
  ...
  segue válida.
- [x] **Rota unificada (Diego confirmou logado, 15/09):** `/vendas/{id}/detalhe` aceita **pack_id e
  order_id**. Usar o pack quando existir — o order abre só aquele pedido, e o pacote pode ter
  outros. `chave` em `pedidos-faturamento.ts` já é `pack_id ?? order_id`.
...
- [x] Todas passam a usar `urlVendaML(packOuOrderId)`, com o pack quando houver. O helper do
...
- [x] **Calcular o delta por vendedor** — **especificado na
...
- [x] **Medir a cobertura no universo real do Sonar** (termo e EAN) e o `N` elegível por consulta —
      ...
      **"Mercado Livre Brasil"** — que renderiza R$ 100,7 mi/mês de faturamento de nicho com
      ...
      Pendências abertas pelo spike:
      - [x] 3.3 contava cobertura sobre os sobreviventes, não sobre a amostra — **corrigido**
            (Errata 2 do contrato): o total da amostra virou parâmetro e o campo reporta as duas
...
- [x] **Sugestão de categoria pela ficha de catálogo (pré-publicação)** (2026-08-22, [ADR-0131](decisions/0131-sugestao-categoria-pela-ficha-catalogo.md)) — motivado pelo lote 21 (Eucerin Aquaphor 55ml publicado em "Bebês > Cremes, Pomadas e Óleos" enquanto a única ficha de catálogo do GTIN vive no domínio corporal, `MLB-BODY_SKIN_CARE_PRODUCTS`, 7 vendedores competindo): `ficha_divergente` só era descoberto depois de publicado, exigindo republicação manual. Estende o padrão não-vinculante do ADR-0057 (sugestão do concorrente) para uma 2ª fonte: a categoria cujo domínio bate com a ficha de catálogo do GTIN, quando diverge da categoria escolhida pelo resolver — **nunca aplicada automaticamente** (mesmo racional do ADR-0054 Fase 2). Contratos de API confirmados com token real em 2026-08-22 (`GET /categories/{id}` → `settings.catalog_domain`; `GET /products/{id}/items` → `results[].category_id`). Implementado em 6 tasks: (1) migration `20260822201053_sugestao_categoria_catalogo.sql` — 3 colunas aditivas em `familias` (`catalogo_categoria_sugerida_id/nome/vendedores`); (2) funções puras em `_shared/ml/catalogo.ts` (`montarEsperadoPrePublicacao`, `deveSugerirCategoriaPorFicha`, `buscarCategoriaFicha`); (3) `buscarDominioCategoria` em `_shared/ml/domain-discovery.ts` (espelho de `buscarNomeCategoria`, cache Redis 30d); (4) `process-familia/sugestao-catalogo.ts` (`calcularSugestaoCatalogo`) fiado no `index.ts`, só no fluxo CREATE (early-return do UPDATE parcial intocado), best-effort (falha de rede/ficha/domínio não persiste nada e não lança); (5) `vincular-catalogo` cita a categoria sugerida no alerta Telegram de `ficha_divergente` (`CatalogoNoMatchAlerta.categoriaSugerida`); (6) card `SugestaoCatalogo` em `card-categoria.tsx`, renderizado direto da row (sem `atributos-familia`, sem rede), com dedupe quando a sugestão do catálogo coincide com a do concorrente. Suíte inteira verde (405 arquivos/3739 testes na Task 6), `pnpm lint`/`tsc -b --force` limpos em cada task. Migration aplicada (`supabase db push`) e `process-familia` (v157) + `vincular-catalogo` (v59) deployadas em 2026-08-22, já com a correção pós-revisão (`d309619c`); frontend no ar pelo Render no mesmo dia.
- [x] **Insights do nicho (Sonar): pódio de rivais vira duplo, faixas de preço saem** (2026-08-27, [Errata 1 do ADR-0124](decisions/0124-veredito-de-oportunidade-do-sonar.md#errata-1-2026-08-27--o-pódio-vira-duplo-e-as-faixas-de-preço-morrem)) — o operador rejeitou 2 dos 3 mini-cards do Adendo 2026-08-21: faixas de preço (tercil sobre embalagens diferentes não descreve nicho) e o rótulo de vendedor no pódio (Apify raramente traz `vendedor`, então a linha em destaque exibia "sem rótulo" repetido). Os dois cards viram um só, "Pódio de rivais", com duas colunas — faturamento e visitas — cada anúncio identificado pelo título, nunca pela loja. Nova função `rivaisPodioVisitas(vendas, visitasPorItem)` em `veredito-sonar.ts`, autônoma e **sem** o filtro `vendidos != null` de `rivaisPodio`: se herdasse, o pódio de visitas nasceria vazio na maioria das consultas, porque os anúncios mais visitados costumam não ter "+N vendidos" exibido pelo ML (medido: líder de visitas da consulta real com 290 visitas não aparecia no pódio de faturamento). `rivaisPodio`/`calcularVereditoAnuncios` intocados de propósito — evita mudar a assinatura usada pela suíte inteira e pelo `scripts/sonar-gabarito-verificar.mjs`. `fraseRivaisPodio` troca a lista de vendedores por "Líder por faturamento: `<título>` (≈ `<valor>`)". `FaixaPreco`/`FaixasPrecoAmostra`/`faixasPrecoAmostra` deletados. `VereditoSonar` ganha prop `visitasPorItem` (Map já montado em `PulseSonar.tsx`, sem chamada de rede nova). TDD: 5 casos novos em `veredito-sonar.test.ts` cobrindo elegibilidade (item com visitas sem vendidos entra; visitas 0 ou Map `null` exclui; `item_id` nulo exclui; top 5; vazio). Suíte inteira 431 arquivos/4051 testes verde, lint 0 erros (15 warnings pré-existentes), `tsc -b --force` limpo. Só frontend, sem migration, sem deploy de edge function.
- [x] **Pódio de rivais (Sonar): placar com vendidos, link para o ML e hierarquia tipográfica** (2026-08-27, [Errata 1 do ADR-0124](decisions/0124-veredito-de-oportunidade-do-sonar.md#refinamento-do-card-2026-08-27-mesma-errata)) — revisão visual reprovou o primeiro corte do card duplo: tudo em `text-xs muted`, sem hierarquia, e o valor que ordena o ranking perdido no fim da linha. O card virou placar (`grid-cols-[1rem_1fr_auto]`, posição fixa, valor à direita em `tabular-nums`, `min-w-0` nas células para o título longo não empurrar o valor para fora). Subcomponente `PodioColuna` recebe strings já formatadas — as duas colunas diferem no texto, não na estrutura, então o layout é garantidamente idêntico. Título do anúncio virou link (`linkDoAnuncio`, nova aba); `href` resolvido no lib para o caso "fantasma sem `item_id`" ser testável fora do render. `+N vendidos` entra na coluna de faturamento (o `+` é obrigatório: faixa piso do ML). Visitas trocam `fmtMilhar` por `fmtInt` — a abreviação exibia 1.100 e 1.900 como o mesmo "1 mil" e fazia posições distintas parecerem empatadas. Faturamento perde centavos (falsa precisão sobre estimativa). Primeiro teste de render do `veredito-sonar.tsx` (link, ausência de link, `+N vendidos`). 432 arquivos/4057 testes verdes, lint 0 erros, `tsc -b --force` limpo. Só frontend.
- [x] **Disputa do Sonar ganha caminho B: concentração por anúncio quando o rótulo não cobre a amostra** (2026-08-27, [ADR-0137](decisions/0137-sonar-disputa-caminho-b-concentracao-por-anuncio.md)) — motivado pela consulta real "Latas Ninho Nestle Zero Lactose 700gr": 5/5 rivais do pódio sem rótulo de loja, e a trava de cobertura (D10/ADR-0127) derrubava Disputa e Tração juntas mesmo com a amostra cheia de dado utilizável (vendidos, preço, Full por anúncio). Caminho A (pulverização por rótulo) segue titular e único que declara disputa 'bom'; caminho B entra só quando a cobertura de rótulo fica abaixo de 50% e mede por **concentração por anúncio** (`concentracaoAmostra`, `veredito-sonar.ts`): share do anúncio líder no faturamento medido (`vendidos×preço` dos elegíveis) — dominante quando `top1 ≥ max(30%, 2× share uniforme)`, exige ≥5 elegíveis. Teto do caminho B é sempre 'medio' (nunca 'bom' — anúncios da mesma loja sem rótulo contam como rivais separados, então a concentração por anúncio subestima a concentração real). `parcial` foi redefinido: agora significa "nenhum caminho da Disputa mediu" (mais o caso pré-existente de Full não medido no caminho A) — `tracao == null` sozinho deixou de causar parcial, porque é sempre null quando o caminho B está ativo (Tração continua exigindo rótulo; escala R$/rótulo não transfere sem segunda calibração). Cortes calibrados sobre os 3 fixtures-gabarito com rótulos censurados: EUCERIN top1 36,8% → ruim; facial Full 100% → ruim; oxford top1 19,6%/Full 21% → medio (teto). `scripts/sonar-gabarito-verificar.mjs` ganhou a 5ª verificação (rótulos censurados, caminho B forçado) confirmando esse gabarito, sem alterar as 4 variantes existentes. TDD: 10 casos novos em `veredito-sonar.test.ts` (`concentracaoAmostra` isolada + `calcularVereditoAnuncios` no caminho B), e 3 describes pré-existentes que testavam a trava D10 antiga foram atualizados — a amostra que antes só pontuava Demanda (poucos rótulos, mas cheia de vendas) agora mede Disputa pelo caminho B, sai de `parcial` e destrava a Entrada — parando em "média". A **Errata 1 do ADR-0137** fechou um furo encontrado na revisão: o teto 'medio' do caminho B não chegava ao veredito, porque com a Tração fora `fatores` tem 2 itens, `maximo = 4` e `soma >= maximo - 1` aprovava disputa 🟡 (3) e 🟢 (4) igualmente — o teto ficava invisível exatamente em "alta", a única faixa que significa comprar estoque, e o `PISO_FATORES_ALTA` passava por tecnicalidade. "Alta" passou a exigir `disputa.caminho === 'rotulo'`. Suíte inteira 432 arquivos/4066 testes verdes, lint 0 erros (15 warnings pré-existentes), `tsc -b --force` limpo. Validado na tela (Playwright, sessão isolada, payload dos fixtures com rótulos censurados injetado por `route`): oxford censurado renderiza "Oportunidade média · entrada aberta" com Disputa "o líder leva 20% do faturamento medido (18 anúncios com venda) · 21% Full"; EUCERIN censurado renderiza "Demanda forte · entrada fechada" com destravar "com o líder abaixo de 30% ... hoje: 37%". Só frontend, sem migration, sem deploy de edge function.
- [x] **Landing page do PubliAI reconstruída — vende a operação, não o software** (2026-09-05, [ADR-0152](decisions/0152-landing-operacao-marketplace.md)) — a landing anterior (`docs/brand/landing/index.html`) vendia o PubliAI como **SaaS**, com seção "Planos" placeholder, contradizendo frontalmente a decisão comercial de 30/08/2026 registrada em `apresentacao/G-Pendencias.md` ("a Daludi trabalha exclusivamente com prestação de serviço e não licencia produto"). A entrevista resolveu 12 pontos e a página foi refeita: 13 seções, HTML único autocontido (sem build, sem framework, zero recurso externo), tema dark indigo→violeta, e a tabela de **Modalidades** no lugar de "Planos". **Capturas reais da org AVIL** (dashboard 90 dias, publicados 30 dias, financeiro 30 dias, revisão) — Radar/Sonar seguem da DSA porque a AVIL não tem o módulo Pulse, e a viabilidade continua sendo simulação genérica porque a tela é uma calculadora e não mostra dado de operação. **Nenhum original sem tarja entrou no repositório**: os títulos e códigos de produto são cobertos no DOM *antes* do screenshot. Três exposições de dado de cliente foram barradas na inspeção visual, uma a uma, apesar de o relatório do agente de redação declarar tudo limpo: `telegram.png` expunha um `MLB-…` que abriria o anúncio real do cliente para qualquer visitante, mais um EAN e a URL de um pedido; `revisao-completa.png` mostrava a foto do produto; e os EANs do `radar.png` tinham escapado porque minha instrução de redação dizia "não tarjar números" — o EAN identifica o produto melhor que o título. **Cinco defeitos funcionais achados só porque a verificação foi refeita à mão, todos reportados como prontos:** (1) a captura do Sonar tinha `loading="lazy"` dentro de painel `hidden` e **nunca carregava** — quem clicava na aba via um espaço vazio, e nem rolar destravava; (2) o formulário exibia "Recebemos seus produtos" para envios **recusados**, porque o FormSubmit responde `HTTP 200 {"success":"false"}` e o handler só checava `response.ok` — lead perdido em silêncio no único ponto de conversão; (3) o fallback de WhatsApp usava `window.open` depois de um `await`, que o Safari bloqueia, falhando exatamente quando era acionado; (4) num flex container o `padding-bottom` **não** estende a área do `position:sticky` (quem limita é o content box), então o 6º card do ciclo nunca grudava e a última etapa recebia menos tempo de leitura; (5) `object-fit:cover` cortava as capturas **pelos lados**, decepando justamente os números que elas existem para provar. **D10 revisada:** o pin com lista lateral virou empilhamento de cards `sticky top:88px`, mecânica medida no site de referência (`aiautomationsociety.ai`), que resolveu junto as duas queixas que o pin não atendia — captura pequena e sensação de scroll travado. Padrão de evidência do H-Briefing valendo integralmente: nenhum percentual sem denominador, só os rótulos "markup (lucro ÷ custo)" e "margem s/ venda", todo número com janela e data de leitura, e o bloco "o que não prometemos" como seção visível. Lighthouse **96 performance / 100 acessibilidade / 100 boas práticas / 100 SEO**, LCP 2,7s, CLS 0,006, zero erro de JS; empilhamento medido em 9 posições de scroll e mobile 390px sem rolagem lateral. Artefato de marketing isolado — **não faz parte do build do frontend**, não é rota da SPA, sem migration e sem deploy de edge function. **Pendências:** o domínio ainda não está decidido (`publiai.daludi.com.br` **não serve** — a raiz já é o app em produção), e a ativação do FormSubmit vale por domínio de origem, então terá de ser refeita a partir do host definitivo.
- [x] **Diálogo de kit: categoria de origem visível e caminho completo nos resultados** (2026-09-10) — achado do Diego ao usar o "Trocar categoria" do kit (ADR-0151): o diálogo mostrava só o nome da categoria escolhida, nunca a categoria em que o produto-base já está — trocar era às cegas — e os candidatos da busca vinham como um nome solto ("Leite", "Leite em Pó"), sem o ramo que os distingue. Agora o diálogo abre com "Categoria do produto: Bebês › Alimentação e Amamentação › …" (a que o kit herda quando não há override), o override aparece como "Categoria do kit:" no mesmo formato, e cada candidato da busca mostra o caminho inteiro. O caminho vem de `GET /categories/{id}` da API pública do ML (sem token, CORS liberado, cache de promessa por sessão em `src/lib/caminho-categoria-ml.ts`) — deliberadamente sem passar por edge function, que custaria um deploy para uma decoração de UI; falha de rede degrada para o nome simples, nunca esconde a categoria. Só frontend. 3 testes novos (caminho da base, caminho dos candidatos, base sem categoria definida); `preflight:static` verde.
- [x] **Mesmo caminho completo de categoria no card da Revisão** (2026-09-10) — Diego pediu o tratamento do diálogo de kit também em `CardCategoria`: a categoria já definida e cada candidato da busca agora mostram o ramo do ML acima do nome (`Alimentos e Bebidas › Mercearia ›` + `Leite em Pó`), empilhado em vez de inline porque o card tem 200px fixos. O fetch saiu do componente do kit para o hook `useCaminhoCategoria` (`src/hooks/useCaminhoCategoria.ts`), que devolve ancestrais e folha separados e deixa o layout com quem renderiza. As sugestões (concorrente e catálogo) seguem sem caminho — não foram pedidas e o card é estreito. 3 testes novos; suíte cheia (5210) e `preflight:static` verdes.
- [x] **Mensagens (Faturamento): botão "Dispensar" tira conversa de "Aguardando" sem resposta** (2026-09-10) — caso motivador: pedido em mediação no Mercado Livre responde 403 `blocked_by_mediation`, deixando a conversa presa em "Aguardando resposta" para sempre. Reusa a RPC já existente `marcar_mensagens_lidas(p_pack_id)`; nova migration `20260910124702_mensagens_dispensar_conversa.sql` faz `contar_conversas_aguardando()` respeitar `lida`. `aguardando` em `src/lib/mensagens.ts` passa a exigir `ultima.lida === false` além de `direcao === 'recebida'` e pedido não cancelado; `dispensarConversa(pack_id)` exportada chamando a RPC. Botão "Dispensar" (`aba-mensagens.tsx`) ao lado de "Sugerir resposta (IA)", visível só quando `c.aguardando`, invalida as mesmas queries de `responder()`. Mensagem nova do comprador reabre a conversa sozinha (entra com `lida=false`). Testes novos em `mensagens-conversas.test.ts` e `aba-mensagens.test.tsx`. Na mesma entrega, `mensagemErroEnvioML` (`_shared/faturamento/mensagens-io.ts`) passou a traduzir o 403 `blocked_by_mediation` do ML, que antes chegava à tela como JSON cru — por tocar `_shared/`, exigiu redeploy de `responder-mensagem`, `sync-mensagem` e `backfill-faturamento`, além do `db push` da migration.
- [x] **Atributo texto-livre com sugestões: as opções passam a aparecer no prompt da IA** (2026-09-10, [adendo ao ADR-0052](decisions/0052-camada2-atributos-ia-first-com-fallback.md)) — criar kit do Ninho na categoria "Leite Infantil" falhava com "exige atributos que não foram resolvidos: Formato da fórmula infantil". Causa reproduzida contra o schema real do ML e o modelo real: `BABY_FORMULA_FORMAT` é obrigatório, `value_type=string` (texto-livre desde a correção de 2026-07-10) e traz sugestões "Em pó"/"Líquida" que o prompt nunca mostrava — a IA não tinha pista do que copiar e omitia, mesmo com "Em Pó" no título. Agora o prompt lista as sugestões, só pelo nome e mandando responder o texto e nunca o código (emitir `id = nome` faria a IA responder "1358247", que a regra de ouro rejeita — a falha visível viraria silenciosa; achado da revisão do Fable ao plano). `validarRespostaAtributos` intacta: sugestão sem lastro no título/descrição continua rejeitada. Afeta process-familia, definir-categoria-familia e criar-kit-vinculado (deploy das três). 2 testes novos + prova ponta a ponta: o gate do kit sai de ["Formato da fórmula infantil"] para [].
- [x] **Erro do gate de kit passa a dizer o que fazer** (2026-09-10) — a mensagem listava só o nome do atributo faltante ("Formato da fórmula infantil") e o operador não tinha como saber quais valores a categoria aceita nem qual era a saída; o diálogo de kit não tem o editor "Complete para publicar" da Revisão (ADR-0151 D-3/D-4), então a frase do erro é a única orientação. Agora anexa os valores aceitos (até 6, com reticência) e a instrução ("inclua no título ou na descrição do produto-base, ou escolha outra categoria"). Separado do caso em que a validação não pôde rodar (sentinela por falha de rede/IA), que antes caía na mesma frase e mandava editar o produto por um problema de infraestrutura — agora diz para tentar de novo. 5 testes novos; suíte cheia (5220) e preflight:static verdes.
...
- [x] **Padrão único de "o sistema está processando, aguarde"** (2026-09-17, [ADR-0163](decisions/0163-padrao-de-espera-em-dialogos.md)) — pedido do Diego a partir do "raio que fica indo e voltando" da tela de Revisão: os dois efeitos que já existiam (a aura `glow-effect-sombra` e a barra `track-indeterminate`) estavam aplicados a esmo em 5 pontos, e em 37 lugares o operador clicava e esperava sem retorno além de um botão desabilitado. A mecânica virou uma prop `processando` em `DialogContent`/`AlertDialogContent` (liga aura, barra e `aria-busy` de uma vez; `destrutivo` suprime só a aura, porque brilho que cicla três cores lê como comemoração num "Excluir"). **Duas revisões do Fable cortaram a entrega de 28 pontos aplicados para 18**: a primeira condenou 16 por enfeite — entre eles um modal cujo `processando` nunca aparecia (o `AlertDialogAction` fecha no clique sem `preventDefault`, então era código morto que passou por três revisores) e dois botões cuja barra `width:100%` quebrava linha em container `flex-wrap` e saltava a toolbar; a segunda tirou mais dois que não falam com canal nenhum. Critério que ficou no contrato de motion: aura+barra só quando (a) a operação fala com Mercado Livre, storage, fila, IA ou monta relatório paginado **e** (b) o modal fica de pé durante a espera — ação local de uma chamada continua sendo spinner no botão, como o §9 já mandava. Junto veio a mudança de comportamento das 6 confirmações de Publicados (migrar, pausar, republicar, remover, remover incompleta, refazer kit), que fechavam no clique e agora seguram abertas até o ML responder. Dois bugs reais achados no caminho: o `finally` incondicional dos handlers permitia, via ESC, que a resolução de uma linha reabilitasse o botão de outra e **disparasse a ação duas vezes no Mercado Livre** (a mesma race apareceu nas remoções de foto, que sequer tinham estado de pendência); e falha ao recarregar o kit depois de encerrado mostrava "Falha ao encerrar kit" quando o kit **já tinha sido encerrado** — antes esse caminho era uma rejeição não tratada, silenciosa, porque o `try/catch` do `MutationObserver` do react-query não alcança nada depois do primeiro `await` de um `onSuccess: async`. Só frontend, nenhum deploy de Edge Function.
- [x] **Percentual de gestão por faixa regressiva de faturamento, por organização** (2026-09-19, [ADR-0165](decisions/0165-faixas-regressivas-por-organizacao.md)) — Diego apontou que a aba Cobrança (ADR-0158) não batia com a página pública, que anuncia percentual regressivo por faixa de faturamento (até 100K/100–300K/300–500K/+500K); o sistema real só gravava um `revenue_bps` único por contrato, sem faixa, e nada travava campo por modalidade. Medido antes de mexer: Daludi Shop e DSA (modalidade 1) cobravam Sonar do cliente indevidamente. `revenue_bps` virou 4 colunas fixas (`revenue_bps_t1..t4`, cortes fixos, só o percentual dentro deles é negociável por org); a faixa aplicada é escolhida automaticamente todo mês pelo faturamento líquido (mesma base que já existia, não é base nova). Modalidade 1 nunca cobra Sonar, modalidade 2 nunca tem infraestrutura separada — trava por `CHECK` de banco e validação na RPC/TS, rejeita com erro, nunca normaliza em silêncio. A migration corrigiu Daludi Shop/DSA no mesmo movimento. Trigger de imutabilidade da tabela desabilitado só durante o backfill, dentro de uma transação com asserção LOUD. **A revisão final de branch achou e reproduziu com um probe** um bug real: o backfill zera o termo retroativamente, mas `platform_sonar_deliveries` é append-only — a linha "Consultas Sonar" sumia do preview enquanto o valor continuava no total (demonstrativo que não fecha). Corrigido antes do merge, com teste de regressão e reverificação por 80 combinações de organização×mês. `billing.ts` (cálculo de fee duplicado em TS, sem importador de produção) removido. Plano de 11 tarefas via subagent-driven-development, cada uma com implementação + revisão (2 delas exigiram 2 rodadas de correção); revisão final de branch por um modelo separado achou o Critical acima. **Validado em produção em 2026-09-19**: as 4 queries de medição bateram exatamente com o previsto antes do `db push`; migrations aplicadas, `platform-admin`/`materializar-metricas` redeployadas (v6/v2), merge fast-forward na `main`. Divergência bruto (página pública) × líquido (base real do corte, sempre a favor da Daludi) documentada no ADR como comportamento pré-existente, não regressão desta entrega.
- [x] **Fix: salvar o Bot token do Telegram falhava com "[object Object]"** (2026-09-19) — Diego reportou o erro na tela Configurações → Alertas no Telegram. Causa raiz: a migration `20260822131053` (achado de segurança F5) revogou o SELECT de `configuracoes.telegram_bot_token` e reconcedeu só nas colunas não-secretas — correto para leitura, mas o save fazia `upsert({..., telegram_bot_token}, {onConflict:'org_id'})`, que vira `INSERT ... ON CONFLICT DO UPDATE SET col=EXCLUDED.col`; o Postgres exige SELECT na coluna pra resolver esse `EXCLUDED`, então qualquer save com token novo batia em `42501 permission denied`. Reproduzido ao vivo (SQL sandbox com rollback, JWT do Diego) antes de mexer em qualquer código. Fable revisou o plano inicial (RPC `security definer`) e apontou uma saída mais simples: um `UPDATE` puro (não `ON CONFLICT`) só lê `org_id` no WHERE e escreve a coluna — não precisa de SELECT nela, não precisa de migration nova. Confirmado com o mesmo sandbox. Fix: `salvarTelegramConfig` (`src/lib/queries.ts`) faz o upsert de `telegram_chat_id`/`telegram_ativo` como antes (garante a linha) e só quando há token, um `update({telegram_bot_token}).eq('org_id', orgId)` separado — ordem importa (upsert primeiro), porque um update num org_id inexistente devolve 0 linhas sem erro e perderia o token em silêncio numa org nova. De carona, `config-telegram.tsx` ganhou um helper `mensagemErro()` pra nunca mais mostrar "[object Object]" quando o erro não for `instanceof Error`. 3 testes novos (`queries-telegram.test.ts`), suíte inteira (532 arquivos/5322 testes) verde, build e lint limpos. Fable revisou plano e diff final antes do merge. Só frontend — sem migration, sem deploy de Edge Function.
