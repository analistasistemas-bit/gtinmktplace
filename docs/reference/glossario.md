...
## Nota fiscal (ADR-0114 — aceito, implementação pendente; **v1 é Simples Nacional apenas**)

| Termo | Definição |
|---|---|
| **NF-e** | Nota Fiscal eletrônica **modelo 55** (mercadoria) das vendas feitas pelo PubliAI. NFS-e (serviço) está fora do escopo — o domínio do PubliAI (família, variação, SKU, estoque) não comporta serviço. |
| **Etapa fiscal (A e B)** | Emitir NF-e são duas coisas distintas: **(A) decidir** o que vai escrito (`NCM`, `CFOP`, `CST`/`CSOSN`, `orig`, base, alíquota, DIFAL) e **(B) assinar, transmitir e receber o protocolo**. O PubliAI faz **só a B**. Nenhum software de transmissão faz a A — ela é decisão do contador do cliente. |
| **Parâmetros fiscais do produto** | `ncm`, `csosn`, `orig_nfe` e `cest` (opcional), gravados **na família**, nunca na variação — variações se distinguem por cor, e cor não muda NCM nem CSOSN. Família sem os três obrigatórios **não emite**: falha LOUD com o campo faltando nomeado, como pendência na Revisão. CST de PIS/COFINS e natureza da operação ficam **na org** (no Simples são praticamente constantes). |
| **Logística coberta** | As modalidades de envio que a v1 emite: `drop_off`, `xd_drop_off`, `cross_docking`, `xd_same_day` — as que têm `invoice_pending`. **Full, Flex, Turbo e ME1 estão fora**, e a trava aparece **na habilitação do módulo**, mostrando quantas vendas ficariam sem nota antes de o super-admin ligar. Venda fora do escopo é marcada, nunca some em silêncio (ADR-0114 D-15). |
| **`orig` (NF-e)** | Campo da NF-e que codifica a procedência da mercadoria em **0 a 8**. ⚠️ **Não é** o **ORIGEM** da planilha (`NACIONAL`/`IMPORTADO`, base da alíquota de markup, ADR-0055/0107). Nomes parecidos, conceitos distintos; usar um como o outro emite nota errada. |
