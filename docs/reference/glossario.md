...
| **Parâmetros fiscais do produto** | `ncm`, `cst_csosn`, `orig_nfe` e `cest` (opcional), gravados **na família**, nunca na variação — variações se distinguem por cor, e cor não muda NCM nem CST. Família sem os três obrigatórios **não emite**: falha LOUD com o campo faltando nomeado, como pendência na Revisão. |
