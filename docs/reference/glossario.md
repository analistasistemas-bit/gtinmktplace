...
| **Sonar** | Aba do Pulse para garimpo de sortimento (ADR-0120): pesquisa de nicho por termo, on-demand com cache de 24h, só API oficial. Par do Radar: o Radar vigia o que a org já vende; o Sonar varre o que ela ainda não vende. Cobertura limitada a produtos com ficha de catálogo. |
| **Prospecção (ex-"garimpo" de sortimento)** | A decisão que o Sonar serve: "vale cadastrar este produto?" — respondida com demanda (visitas 30d, ranking, termos em alta), barreira competitiva (ofertas, vendedores, frete grátis) e margem simulada com custo hipotético informado pelo operador. Acontece ANTES de existir planilha. Rotulado "Prospectar" na UI desde 19/08/2026; ADRs e código anteriores usam "garimpo" para o mesmo conceito. |
...
| **Anúncio principal** (Sonar) | Quando uma ficha de catálogo casa com mais de um anúncio na amostra da Apify, o anúncio de maior `vendidos` (desempate: menor posição). Faturamento, avaliação, desconto e selo mostrados na ficha vêm todos DESTE anúncio — nunca colados de anúncios diferentes (ADR-0125/D1). |
| **Posição orgânica** (Sonar) | Menor posição entre os anúncios casados com a ficha que NÃO são patrocinados — anúncio pago não conta como ranqueamento orgânico. Fichas só com anúncios patrocinados casados mostram "—" (ADR-0125). |
