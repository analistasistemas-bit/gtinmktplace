...
- **Chave composta `"<label>::<tela>"`** — só para os 3 labels confirmados divergentes entre
  pipelines de cálculo (ver "Achado colateral"): `"Pedidos"`, `"Ticket médio"` e `"Faturamento"`
  (achado ao escrever o plano: Publicados `dashboard-publicados.tsx:73` usa `totais.faturamento`
  vindo de `calcularResumo()`, Faturamento/aba-vendas `aba-vendas.tsx:372` usa `kpis.bruto`
  vindo de `calcularKpisPedidos()` — mesmo mecanismo de divergência já confirmado, um pack de
  status misto pode ter o bruto incluído por inteiro num pipeline e excluído por inteiro no
  outro). Cada tela recebe texto específico mencionando a fonte real (ex., em Publicados: "conta
  como faturável se qualquer item do pedido foi faturado"; em Dashboard/Faturamento: "conta como
  faturável pelo status do pedido no checkout").
...
- `infoKey?: string` — chave a buscar no dicionário (default: usa o próprio `label`). Só passada
  explicitamente pelos 3 labels de chave composta (`"Pedidos"`, `"Ticket médio"`,
  `"Faturamento"`).
...
leitura errou o alcance) confirmou que `"Pedidos"`, `"Ticket médio"` e `"Faturamento"` são
calculados por **dois pipelines diferentes**:
...
granularidade do filtro "faturável" em `"Pedidos"`/`"Ticket médio"`/`"Faturamento"`.
