...
**Bloqueio da escrita direta (D-20):** trigger `before update of estoque on variacoes` que
levanta exceção quando `auth.uid()` não é nulo. Preserva o `service_role` (workers e re-ingest
de planilha seguem escrevendo normalmente) e não pode ser contornado pelo PostgREST.
...
cobrindo D-1..D-20 (e os cortes declarados: não há ajuste manual de estoque pelo app; devolução não é
