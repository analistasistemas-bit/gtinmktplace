...
- `registrar_entrada(p_org, p_codigo, p_qtd, p_custo, p_doc, p_obs) returns integer` — movimento
  `entrada`, soma no saldo canônico e, se `p_custo` não for nulo, sobrescreve `variacoes.custo` (D-9).
...
cobrindo D-1..D-15, com as alternativas rejeitadas registradas: `lote_id` nullable (custo verificado
