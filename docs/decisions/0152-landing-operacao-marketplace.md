...
| 4 | Domínio onde a página vai ao ar | Diego | ✅ **resolvida em 15/09/2026** — landing no ar em <https://publiai.daludi.com.br> (200, ~70 KB), servida pela Vercel com integração GitHub e Root Directory `docs/brand/landing`. O app migrou para <https://app.publiai.daludi.com.br> (Render), então a justificativa anterior ("a raiz já está ocupada pelo app", verificada em 05/09/2026) deixou de valer. Destrava a `og:image` e a URL canônica |
...
| 6 | **Ativação do FormSubmit** | Diego | ⚠️ **não confirmada.** A hipótese deixada em aberto aqui se concretizou: a ativação de 05/09/2026 foi registrada para `https://publiai.daludi.com.br/`, e é exatamente nesse host que a landing ficou (linha 4). O domínio da ativação passou a bater com o domínio da página, então a reativação provavelmente **não** é mais necessária — mas **nenhum envio real foi feito desde a publicação**, e a ativação do FormSubmit só se prova enviando. Ação: um envio de teste rotulado a partir da página no ar; a resposta AJAX distingue sucesso da mensagem "needs Activation" (medida em 05/09/2026 a partir de `localhost:8099`). Até lá, tratar o formulário como **não verificado** — lead não chega em silêncio |
