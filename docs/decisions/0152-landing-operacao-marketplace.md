...
| 6 | **Ativação do FormSubmit** | Diego | ✅ **confirmada em 25/09/2026** — envio de teste rotulado pela página no ar voltou `{"success":"true"}` (chegada no `sac@` conferida pelo Diego) |
| 8 | Aviso de lead no Telegram | — | ✅ 25/09/2026 — depois que o FormSubmit aceita o lead, o JS da página chama a edge `lead-landing-telegram` (best-effort, `text/plain` sem preflight). **Não usar o `_webhook` do FormSubmit:** com ele o FormSubmit responde 500 e o lead não sai (medido em 25/09/2026, mesma página, com e sem o campo). Ela avisa no Telegram do Diego via bot dedicado `@Publiai_leads_bot` (ver `docs/reference/edge-functions.md`). Best-effort: se falhar, o e-mail continua chegando |
