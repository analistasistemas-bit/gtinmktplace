...
>
> **Emendado de novo pelo [Spike 049](../spikes/049-buy-box-do-radar-o-que-e-mensuravel.md)
> (2026-08-29):** a emenda "identificar por vendedor" do Spike 039 **não resolve**, porque a causa
> não é cobertura e sim **opt-in de catálogo** — os anúncios da org têm `catalog_listing: false` e
> por isso o `seller_id` dela não está na ponte. **AVIL: 0 de 137.** Existe ground truth de buy-box
...
| D-5 | **Sonar:** após consultar um produto, o botão **Análise PubliAI** gera um relatório de 7 seções. **As seções 2, 3 e 7 têm contrato normativo em [`docs/reference/contrato-analise-publiai-secoes-2-3-7.md`](../reference/contrato-analise-publiai-secoes-2-3-7.md)** — campo a campo, com critérios de aceite. As 6 seções de mercado saem imediatamente, sem pedir nada ao operador. A seção 6 (DRE) aparece como bloco que pede custo, origem, peso e dimensões — dados que não existem no banco porque o produto é do concorrente. Relatório parcial é entrega válida: no garimpo o operador frequentemente ainda não tem o custo. |
...
| D-9 | **Top 5 por faturamento estimado é um conjunto único**, lido três vezes: a seção 1 tira dele o preço médio sem extremos, a seção 4 o tabula, a seção 5 mostra os cards. O preço de referência passa a ser o praticado por quem efetivamente vende. Anúncio parado não é referência de mercado. **Emendada pelo [Spike 040](../spikes/040-revisao-adversarial-adr-0141.md), decidida por Diego em 2026-08-28:** o preço médio **em reais** só existe no **modo EAN**, onde a amostra é o mesmo produto. No **modo termo**, a seção 1 se expressa em **percentual / equivalente por unidade** — nunca em valor absoluto. Motivo, que é regra do próprio operador na [ADR-0138](0138-sonar-linguagem-comercial-e-condicao-de-entrada.md): a busca por termo mistura embalagens ("abraçadeira nylon" devolve Kit 50 a R$ 19,06, Kit 500 a R$ 39,90 e Kit 1000 a R$ 77,96 lado a lado), e um alvo em reais sobre esse conjunto é alvo de prejuízo para quem cadastra outro tamanho de kit. A Errata 1 da [ADR-0124](0124-veredito-de-oportunidade-do-sonar.md) já havia matado as faixas de preço do Sonar pelo mesmo motivo. O Top 5 por faturamento **agrava** o problema, porque kits grandes concentram receita e puxam a média. |
| D-10 | **Menos de 5 anúncios ELEGÍVEIS: a média sem extremos não se aplica** e o relatório diz isso explicitamente. **Corrigida pelo Spike 040:** o teste é sobre os anúncios que entram no cálculo (os com faturamento estimado), não sobre o tamanho do nicho — 20 anúncios com 2 elegíveis passariam no teste antigo e a exclusão de extremos deixaria zero elementos. Com 3, excluir o menor e o maior deixaria 1; com 2, nenhum. Nunca se produz uma "média" de 2 elementos nem se completa a amostra. Anúncio sem venda estimada conta como concorrente, mas fica fora do cálculo de faturamento. |
...
| D-16 | **A entrada de peso e dimensões vive no bloco da DRE do relatório**, não em `DialogMargemSonar`. **Reforçada em 2026-08-28 (Diego):** o **peso taxável e o enquadramento de taxa fixa/frete saem da seção 3 e passam para a seção 6**, pelo mesmo motivo — eram o segundo lugar pedindo peso. A seção 3 fica puramente demanda, e a promessa da D-5 (6 seções de mercado sem pedir nada) segue íntegra. Um único lugar para digitar peso: dois divergem. O aviso atual do simulador ("produto ainda sem dimensões cadastradas — margem otimista, sem custo de frete") deixa de valer dentro do relatório, porque ali o frete é real. |
...
| D-28 | **A DRE recusa o cálculo quando o número não é oficial — e o guard nasce junto com ela.** Decidido por Diego em 2026-08-28 (opção A do [Spike 040](../spikes/040-revisao-adversarial-adr-0141.md)). Comissão e frete são obtidos por helpers que hoje devolvem `0` em falha (`listing-prices.ts:17`, `tarifa.ts:24`, `frete.ts:21`) e substituem dimensões ausentes por um pacote default (`frete.ts:27`). **Esses helpers não mudam de contrato** — servem `process-familia` (publicação), `pulse-coletar`, `analisar-viabilidade` e `calcular-tarifa-ml`, e alterá-los quebraria produção. A DRE usa uma **variante que devolve `{ valor, proveniencia }`**, com os helpers atuais virando wrapper fino que colapsa para `0`; fora de `official`, a DRE **não calcula** e diz por quê. O tipo `Proveniencia` já existe (`calculadora-ml.ts:15`) e o padrão tem precedente: o homônimo `buscarFreteVendedor` de `_shared/faturamento/io.ts:193` devolve `number | null` justamente para não inventar zero. **Medido antes de decidir ([Spike 042](../spikes/042-zero-silencioso-quantas-vezes-aconteceu.md)): o zero silencioso nunca ocorreu em produção** — comissão zero em 0 de 229 linhas do Pulse e em 0 de 2.282 vendas pagas; os 94 fretes zero são todos de produtos entre R$ 12,50 e R$ 15,80, abaixo do limite de R$ 79; as 10 comissões zero são de vendas canceladas. É seguro, portanto, que o guard entre **com** a DRE em vez de antes dela: implementá-lo agora criaria código sem chamador. O risco é latente, não inexistente — a API do ML simplesmente respondeu sempre. |
...
## Revisão adversarial — [Spike 040](../spikes/040-revisao-adversarial-adr-0141.md), 2026-08-28
...
- ~~**B-2 — a D-9 reintroduz o preço absoluto**~~ **RESOLVIDO em 2026-08-28 por Diego: preço médio em reais só no modo EAN; percentual no modo termo.** A D-9 e a D-10 já refletem a decisão. Registro do que motivou — a D-9 reintroduzia o preço absoluto que a [ADR-0138](0138-sonar-linguagem-comercial-e-condicao-de-entrada.md)
