# ADR-0096: Código de produto automático no cadastro manual
...
A tabela abaixo é cópia **verbatim** da seção 2 (`## 2. Decisões travadas`) da spec
`docs/superpowers/specs/2026-07-31-codigo-produto-automatico-design.md`.
...
A inicialização virou reaplicável na migration seguinte (`20260731193955`): o `greatest()`
original não incluía o valor corrente de `produto_seq`, então reaplicar a migration sobre uma
base que já reservou números **rebobinaria** a sequência — número reservado nem sempre vira
linha inserida (cadastro que falhou no meio, retry, ou estouro recusado por `derivarCodigos`
queimam números sem deixar rastro). O ratchet (`greatest(produto_seq, …)`) é a mesma forma que
o ramo `p_resync` da RPC já usa, e é obrigatório porque não há rede no banco para isso: um
índice único org-wide sobre os códigos é impossível com os dados atuais (23 grupos duplicados
de `(org_id, codigo_pai)` e 437 de `(org_id, codigo)`).

Uma terceira migration (`20260731194443`) desloca só a Avil para a faixa reservada de D-4.2.
...
### Ponto de injeção dos códigos

`validarProdutoNovo` deixa de validar código. `derivarCodigos(ultimoDaFaixa, qtd)` é função pura
em `_shared/produto/codigos.ts` e devolve `{ codigoPai, codigos[] }`. `montarLinhasProduto` passa
a **receber os códigos como parâmetro** — o payload da request nunca é mutado.
...
- **Número puro sem zeros à esquerda** — manteria o upload de foto quebrado nos produtos novos
  (o match exige `^\d{8}`).
- **Gerar o código no front** — colidiria entre duas abas do operador.
- **`max(codigo) + 1`** — colidiria sob concorrência (dois cadastros simultâneos).
- **Formato configurável por org** (prefixo, largura) — YAGNI, e reabriria o bug que este
  trabalho fecha: um segundo formato voltaria a produzir código fora do contrato `^\d{8}`.
- **Ressincronizar a sequência ao habilitar o módulo** (`set_modulos_org`) — seria otimização
  do primeiro cadastro, não correção: a ressincronização no caminho da colisão (D-4.1) já
  cobre o caso, e isso seria uma segunda cópia da mesma regra.

## Consequências

- O operador do módulo de estoque não digita mais código de produto nem SKU — o sistema gera
  os dois no formato que o upload de foto exige, fechando o canal que produzia SKU inválido
  (ex.: EAN de 13 dígitos colado no campo errado).
- Produto cadastrado por este caminho passa a aceitar foto pelo ícone de câmera na Revisão,
  porque o código gerado casa com o `^\d{8}` do match — efeito colateral esperado, não é o
  objetivo do trabalho, mas é a razão de D-1 não ser negociável.
- `organizations.produto_seq` é uma sequência por org, análoga a `lote_seq`; a Avil está
  deslocada para a faixa `99xxxxxx` (D-4.2), a DSA segue em `1`.
- O caminho de planilha (`ingest-lote`) não muda — continua sem numeração automática (D-7).
