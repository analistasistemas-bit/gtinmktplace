...
- **Sonar — busca por EAN: análise completa pela busca (ADR-0140, 2026-08-28).** A consulta por EAN deixou de ter caminho e view próprios: o código de barras vira termo de busca e percorre o mesmo pipeline da consulta por descrição (veredito, insights, pódio, vendas, visitas, filtros, simulador de margem). Escolha "grátis / com vendidos" removida — toda consulta por EAN é paga (~US$ 0,10, cache 7 dias). Medido: a busca via Apify devolve 20 dos 24 anúncios do EAN, contra 1 pelo lookup de catálogo, e alcança anúncios fora do catálogo. Revoga ADR-0136 D-6. Cruzamento com o catálogo da org mantido, com o Radar afirmado só no positivo (ids de catálogo agora vêm parciais da amostra). Edge `pulse-sonar-ean` removida em 2026-09-07 — a escotilha do D-4 foi encerrada, ver adendo no ADR.
- **[superado pelo ADR-0140] Sonar — busca por EAN/GTIN (ADR-0127 Errata 1, 2026-08-22).** Edge nova `pulse-sonar-ean` + parsers `_shared/pulse/sonar-ean.ts` (validação EAN, interseção por `item_id` restringindo "vendidos" da Apify ao produto do EAN). UI em `PulseSonar.tsx` (escolha grátis/com vendidos, `autoFocus` para leitor físico). Deploy `pulse-sonar-ean` versão 1 ativa em produção. Fix pós-Fable: falha transitória ML não vira tombstone "EAN sem ficha" cacheado — devolve 502 explícito.
...
- **ADR-0153: PubliAI virou PWA instalável, com leitura offline na sessão viva (2026-09-05) — EM PRODUÇÃO.** O manifest e os ícones já existiam; faltava o service worker, sem o qual o Chrome não oferece instalar e não há nada offline. Agora instala no Android e no iPhone (Safari → Adicionar à Tela de Início) e abre sem conexão, servindo o shell do precache (147 arquivos). O service worker **nunca** intercepta cross-origin: `runtimeCaching` vazio, nenhuma resposta do Supabase em cache — resposta autenticada carrega dado de um `org_id`, e cache que não entende RLS pode servir para a conta errada (há teste travando contra isso). No levantamento apareceu um risco que já existia sem relação com PWA: `query-client.ts` não definia `networkMode`, e o padrão do react-query v5 pausa a mutation sem rede e a dispara sozinha ao reconectar — 47 mutations em 18 arquivos, incluindo pausar/reativar anúncio, reprecificar e movimentar estoque. Corrigido com `mutations.networkMode: 'always'` antes de qualquer service worker. Offline vale só para a sessão viva: nenhum dado de organização em disco. Atualização de versão por aviso, nunca automática. Offline deixou de virar falta de permissão (mostra "Sem conexão" em vez de "Sem acesso", e falha ao carregar módulos não esconde mais os menus pagos). Revisado pelo Fable sem bloqueios, com 6 correções aplicadas depois. Web Push levantado e registrado no backlog, fora do escopo.
- **ADR-0154: Kit Virtual do Mercado Livre (2026-09-06) — EM PRODUÇÃO,** mergeado na `main` (`a3439b8c`, CI verde) com schema aplicado, 42 Edge Functions deployadas e validação feita publicando kits reais. Recurso do ML (produção desde out/2025) que agrupa 2 a 6 produtos **distintos** já publicados, referenciados por `user_product_id`, com estoque e preço calculados pelo próprio ML. Entidade própria (`kits_virtuais`/`kits_virtuais_componentes`), fora do pipeline de produto por construção — o kit não é `familias` nem `anuncios_externos`. Preço sempre por desconto automático (nunca reprecificado pelo app); título por template (o ML expande sozinho) e descrição por IA uma vez no preview; foto própria obrigatória, subida ao ML no upload do diálogo (não no publicar); margem exibida como estimativa, nunca bloqueante nem um `0%`/`—` silencioso; imposto por origem sobre a parcela rateada de cada componente; "Refazer kit" encerra e recria (composição imutável no ML); componente de kit publicado bloqueado de remoção/republicação (guard de banco + guard de app em `remover-publicado`); kit aparece na tela Publicados (badge, preço/estoque ao vivo via `/sale_price` e `/user-products/{id}/stock`, sem Pausar/Reativar na v1) e nas vendas (badge, `ml_vendas.kit_item_id`, faturamento intocado). Elegibilidade medida nas duas contas reais (2026-09-06, só leitura): Avil 79 de 147 anúncios no modelo certo (138 UPs elegíveis), DSA 24 de 24 (14 elegíveis) — decide o item ser user product nativo, sem `variations[]`. Sonda de escrita (`POST /items/kits` inválido) devolveu `400` de validação nas duas contas, não `403` de certificação — endpoint autorizado, nenhum kit criado de verdade. Suíte final: 495 arquivos/4865 testes verdes. **Validado publicando de verdade:** três kits criados e encerrados na conta da DSA (`MLB5194780911`, `MLB5194783047`, `MLB5194848525`); o preview previu R$ 41,79 e o ML cobrou R$ 41,79; estoque calculado bateu (`min(12,6) = 6`); a idempotência do CREATE foi confirmada em produção (2ª chamada com a mesma chave devolveu o mesmo `ml_item_id`) e o encerrar também é idempotente. A publicação real revelou quatro defeitos que os testes não pegariam (`thumbnail` exige `secure_url`; `listing_type_id` tem que vir dos componentes; `cause[]` do ML também vem como string; o ML **não** expande o `family_name`, então o título ia truncado ao meio da palavra para o anúncio) e a revisão final achou outros quatro, todos corrigidos antes do merge. **Ainda aberto:** as três incógnitas de tarifa da Decisão 15 — só a primeira venda de kit resolve; até lá a margem aparece rotulada como estimativa. Ver [ADR-0154](decisions/0154-kits-virtuais-mercado-livre.md) e [Spike 036](spikes/036-kits-virtuais-mercado-livre.md).

- **ADR-0158: Central de organizações — carteira agregada, pendências corretas e alíquota explícita (2026-09-07) — EM PRODUÇÃO.** Auditoria da central `/admin` (ADR-0155) achou dois bugs de dado: `platform_resolve_terms` é função escalar e devolvia uma linha toda `NULL` sem condição comercial, então o blocker `commercial_terms_required` era código morto e a prévia saía com `total_cents NULL`; "Pendências" contava buscas Sonar em voo (sempre 0), não os bloqueios do demonstrativo. Corrigido: `platform_billing_preview`/`close` detectam ausência por `v_terms.id is null`, e o fechamento sem condição recusa com erro de negócio (HTTP 422, `commercial_terms_required`) — provado em produção (Avil 2026-09: 1 bloqueio, `total_cents 0`; Avil 2026-08: 7; DSA 2026-08: 4). Ação `wallet` substitui `overview`+`list` (1 chamada; `readOrgMetrics` da Avil caiu de 13 para 5 round-trips, sem mais ler `ml_vendas.raw`); nova RPC `platform_org_cost_catalog` troca a leitura de `variacoes` inteira (8 537 linhas na Avil) pelas só vendidas (6 284, ~115 ms), devolvidas em `jsonb` agregado para não estourar o `max_rows=1000` do PostgREST em silêncio. Alíquota tributária nunca mais presumida (8 %/16 %) — sem `aliquotas_confirmadas_em` o markup sai `null`; as duas organizações já tinham a confirmação, então nenhum número mudou, só o formato (`+43%`). `Organizacoes.tsx` e as 5 abas de `OrganizacaoDetalhe.tsx` redesenhadas no design system do app; o formulário de condições comerciais passou a existir na interface, na aba **Cobrança**. Fora da entrega: contagens operacionais e custo medido do fornecedor (nunca implementados). A suspeita de que a busca Sonar por EAN ficaria fora do ledger foi **refutada** — desde o ADR-0140 a interface não usa mais `pulse-sonar-ean`, e o EAN digitado percorre a rota medida; a função órfã foi removida do repositório e do projeto. **Validado em produção com dados reais em 2026-09-07**, e o primeiro uso achou três defeitos que os testes não pegavam: era impossível salvar condição com implantação maior que zero (escape duplo na regex de `setup_due_month`, ramo que nenhum teste cobria), o teste SQL de fechamento estava abortando na primeira linha por colisão de `ml_vendas` entre arquivos encadeados, e condição com vigência futura era exibida como condição ausente. Todos corrigidos. As duas organizações passaram a ter condição comercial vigente a partir de 2026-10-01 — Avil na modalidade 2 (7% sobre receita) e DSA na modalidade 1 (R$ 600 + 5%), ambas com Sonar a R$ 1,20 e implantação de R$ 3.000; setembro não é faturável, porque vigência retroativa é proibida. Ver [ADR-0158](decisions/0158-central-carteira-agregada-e-pendencias.md) e o [guia de operação](how-to/central-organizacoes.md).

- **ADR-0160: Preço por variação sob User Products (2026-09-08) — EM PRODUÇÃO,** mergeado na `main` (`336a7aeb`, CI verde) com 42 Edge Functions deployadas; sem migration. O Mercado Livre passou a oferecer "Oferecer preço por variação" nos anúncios (o Tecido Oxford Liso 10m, `MLB4831319319`, entre eles). A investigação mostrou que o recurso **é** a migração **UPtin** para User Products: o item original é encerrado (`closed`), cada variação vira um MLB próprio, `variations[]` deixa de existir e enviá-lo passa a devolver 400 — `sold_quantity` é herdado, mas as ordens antigas continuam presas ao item antigo. Isso **corrige a premissa central do ADR-0078**: "preço único entre variações" é regra do modelo **Legacy**, não do ML inteiro. O PubliAI já publicava em User Products desde o ADR-0088, mas colapsava o preço num escalar (`precoFamilia` = 1º `preco_publicacao` da família) por herança do caminho Legacy, achatando em silêncio qualquer preço diferenciado. Agora `EntradaComposicao.precoPorSku` (simétrico a `estoquePorSku`) dá a cada item o seu preço; o split por faixa continua valendo **só** para Legacy (`decidirSplit` ganhou `ehUP`, sinal estrutural de `anuncios_externos_itens`, nunca o cache de formato por categoria). **Três bugs pré-existentes vieram junto, porque destravar o preço sem eles publicaria valores errados com HTTP 200:** (1) o app escrevia durante a janela de migração — o item segue `active` com `variations` povoado, o PUT volta 200 e é descartado porque os clones já nasceram do estado anterior, então o banco gravava "confirmado" enquanto a vitrine subia com o preço velho; agora `buscarItemML` lê `tags` e o conector recusa com `MIGRACAO_EM_ANDAMENTO` (409 retentável) enquanto houver `variations_migration_pending`; (2) o atacado não reaplicava em reprice puro (`efeitosPosComposicao` só rodava com `houveMudanca`), deixando o PxQ calculado sobre o preço antigo; (3) `preco_publicado_ml` nunca era gravado no UPDATE UP — o worker retorna em `rodarUP` antes do bloco que grava no Legacy —, então o badge "preço alterado" de família migrada ficava aceso para sempre depois do primeiro reprice. Atacado e desconto seguem família-level: com preços divergentes ou config de atacado por cor, o caminho UP **recusa LOUD** (`atacado_status='erro'`) em vez de aplicar uma base arbitrária (ADR-0055). Duas revisões independentes do Fable; a segunda pegou dois bugs de dinheiro introduzidos na própria entrega — a trava nova impedia *desligar* o atacado (deixando o PxQ vivo no ML enquanto o app dizia "erro") e disparava com `[]`/`false` que a própria UI grava, o que travaria o atacado para sempre em toda família que passou pelo editor de faixas. **Validação em anúncio real ainda pendente** (o Oxford ficou de fora por decisão do operador). Ver [ADR-0160](decisions/0160-preco-por-variacao-sob-user-products.md). **Errata (2026-09-16):** a menção a "desconto" acima ("Atacado e desconto seguem família-level") saiu do produto — ver ADR-0162; atacado segue exatamente como descrito.

- **ADR-0161: botão "Migrar para preço por variação" no PubliAI (2026-09-08) — EM PRODUÇÃO,** mergeado na `main` (`4ba64bdd`, CI verde), migration `20260908185250` aplicada e 10 Edge Functions deployadas. **Revoga a §4 do ADR-0160 no mesmo dia:** ao ver o roteiro de validação, que alternava entre o painel do ML e o app, Diego recusou — "não faz sentido estar fazendo isso em dois lugares". Além da ergonomia, disparar pelo app **melhora a confiabilidade**: o app fotografa `variations[]` **antes** da migração, e é esse snapshot que permite casar cor → anúncio novo usando só ids que o ML atribuiu àquele item; sem ele o caminho seria a descoberta por título (ADR-0105), que casaria com uma **família irmã** de mesmo título e mesmas cores e adotaria os anúncios do produto errado, com 200 e nenhum sinal. Fluxo: botão admin-only na Publicados → diálogo declarando o que é irreversível → 12 recusas antes de qualquer escrita no ML (produto de outra org, não publicado, **dividido em N anúncios**, sem raiz, já migrando, raiz/família incoerentes, publicando, **componente de kit virtual**, inelegível, snapshot vazio, **duas cores iguais**, claim perdido), cada uma com teste provando que o ML não foi tocado → snapshot + claim atômico → POST → worker acompanha (15 min enquanto o ML cria os filhos, 1 min depois de `migration_completed`, teto de ~6 h, porque a API não tem estado de falha nem webhook) → casamento em cascata → adoção → catálogo reenfileirado, `atacado_status` zerado e push de estoque **só nos SKUs em que `saldo_local ≤ vivo`** → sino. Estado mora na **raiz `anuncios_externos`**, não em `familias` (há uma linha por lote, e a Publicados usa a mais antiga enquanto o ingest usa a mais nova). Guards novos impedem publicar/pausar/reativar/remover/excluir durante a janela. `fundirAnunciosMigrados` faz a venda do anúncio encerrado continuar sendo reconhecida como do PubliAI — sem isso um pedido antigo reprocessado viraria venda de fora, sem código e sem custo (a mesma classe de falha de 2026-08-11, quando 12 unidades venderam sem baixar estoque). **Três revisões do Fable**: a primeira reprovou o plano (estado no lugar errado, casamento inseguro, split/kit virtual/claims ausentes); a última bloqueou a entrega e pegou três defeitos que nenhum teste apanharia — a **trava anti-oversell estava inerte** (a função recebia a lista de SKUs seguros e empurrava o produto inteiro, restaurando unidades vendidas), um **transitório de rede encerrava a migração** em erro definitivo com o ML seguindo em frente, e o **J9 estava escrito no ADR e ausente no código**. **Limitações aceitas:** famílias divididas e produtos em Kit Virtual ficam de fora; o selo "% OFF" some após migrar; a métrica de vendas da tela recomeça do zero. **Validação em anúncio real pendente.** Ver [ADR-0161](decisions/0161-botao-migrar-preco-por-variacao.md).

### 09 a 15/09/2026 — entregas sem ADR novo (todas em produção)

Período de 51 commits que não criou ADR: são extensões e correções dentro de decisões já aceitas
(adendos em 0021, 0052, 0088 e 0151 foram escritos junto com o código). Rastreabilidade pelo SHA.

- **Publicados — varredura de anúncios órfãos (2026-09-10).** Edge nova `varrer-anuncios-orfaos`
  (`672c1d21`), só leitura, sob demanda: confronta os anúncios vivos da conta no ML com **7 fontes**
  de id do banco e devolve o que o app não conhece, classificado em `perdido_do_app` /
  `catalogo_sem_vinculo` / `externo`. Nasceu do incidente do mesmo dia (adendo do ADR-0088): anúncio
  criado pelo app perdeu a linha no banco e seguiu **ativo e vendendo**, invisível ao sistema. O
  primeiro uso real produziu falso alarme — **10 dos 13 "fantasmas" eram anúncios de catálogo
  saudáveis** —, corrigido em `1114a6ca` com as 3 fontes que faltavam (`catalog_listing_id` de
  `variacoes` e de `anuncios_externos_itens`, e `ml_item_id_anterior` da migração do ADR-0161).
  Fail-closed e paginado: uma fonte truncada em 1000 linhas pelo PostgREST transformaria anúncio
  legítimo em órfão (achado do Fable). Mais 2 falsos positivos corrigidos em `2a7dfc5b`.
- **Publicados — publicação incompleta e filtro de catálogo (2026-09-10/11).** Família com anúncio
  vivo no ML mas sem vínculo completo passava por publicada; agora aparece em **vermelho** com
  filtro próprio (`e56ccd13`). Somado ao filtro **"sem vínculo de catálogo"** (`65aa234b`). O texto
  do chip deixou de prometer ação que o não-admin não tem (`b62e8e47`).
- **Catálogo — `sem_produto` e `pendente` deixam de ser estados terminais (2026-09-11).**
  `45c67e35`, adendo do ADR-0021: o ML pode passar a ter a ficha depois, então o retentar volta a
  alcançar esses estados. Ressalvas do Fable aplicadas em `4e891576` (retorno explícito ao operador
  e guarda contra sync concorrente).
- **Faturamento — Sincronizar segue o período da tela, fatiado em 7 dias (2026-09-11).** `1ff97670`
  (+ ressalvas do Fable em `9698ff5e`); `9fedbfe0` faz só a 1ª fatia pagar perguntas/claims/
  mensagens. Junto, três correções em que **falha passava por sucesso na tela**: a do ML
  (`28bce9f9`), a do Mercado Pago, que simplesmente sumia — agora o líquido tem aviso próprio
  (`1017b04d`) — e a premissa errada sobre o próprio Mercado Pago (`5a31be5d`).
- **Mensagens — botão Dispensar (2026-09-10).** `fb556bd9`, migration
  `20260910124702_mensagens_dispensar_conversa.sql`: tira a conversa de "Aguardando" sem resposta
  enviada, caminho para packs que o ML bloqueia. Avisa quando a RPC não alcança o pack (`fef02727`,
  ressalva do Fable). Antes, `98e12cb8` passou a traduzir o **403 `blocked_by_mediation`** do ML em
  vez de mostrar erro cru.
- **Kit vinculado — categoria própria, diferente da base (2026-09-09/10).** Extensão do ADR-0151:
  `categoriaOverride` resolve os atributos pela categoria nova e não pela da base, com UI "Trocar
  categoria" no diálogo. Caminho completo da categoria visível no seletor e no card da Revisão
  (`a2afc4d0`, `787df783`), erro de atributo faltante passando a dizer os valores aceitos
  (`003a016d`) e aura de "processando" enquanto cria e publica (`2632bf3b`).
- **Remoção/exclusão — família User Products (2026-09-10).** Família UP **sem `ml_item_id` mas com
  filho vivo** voltou a ser removível (`31e11382`) e a trava de exclusão passou a enxergar o item
  remoto (`3e2c5ef2`).
- **Mercado Livre — a rota `/vendas/pacote/` morreu (2026-09-15).** `d3e1fc47` + `15f3445d`: o ML
  descontinuou `/vendas/pacote/{id}/detalhe` (301 para a lista com **qualquer** id, real ou
  inventado). A rota viva é `/vendas/{id}/detalhe`, que aceita `pack_id` **e** `order_id`. Estava em
  **três** superfícies — aba Vendas, alerta do Telegram e o atalho novo de Mensagens (`80b56fa5`) —
  e as duas antigas já falhavam calado. Helper único `urlVendaML()`, espelhado em
  `_shared/notificacoes/telegram.ts` (Deno não importa de `src/`); redeploy das 8 functions que o
  importam. O teste do Telegram **afirmava a rota errada como correta**, e por isso não pegou nada.
- **Landing page — versão comercial oficializada e um 404 em produção (2026-09-15).** `72d5781a`
  promoveu a nova versão comercial. Em seguida a página saiu do ar: o projeto Vercel passou a
  publicar pela **integração GitHub** (Root Directory `docs/brand/landing`) e, nesse modo, a Vercel
  avalia o `.vercelignore` da pasta **contra a raiz do repositório** — o `*` da primeira linha
  apagou os 2204 arquivos do repo, o build saiu sem output e o domínio respondeu **404 com o deploy
  marcado Ready**. Corrigido em `ac1d6dcc` trocando por padrões sem âncora. A landing passou a ter
