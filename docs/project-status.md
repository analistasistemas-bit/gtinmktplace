...
- **ADR-0154: Kit Virtual do Mercado Livre (2026-09-06) — schema aplicado e 24 Edge Functions deployadas em produção; branch `worktree-spike-036-kits-virtuais-verificado` aguardando merge.** Recurso do ML (produção desde out/2025) que agrupa 2 a 6 produtos **distintos** já publicados, referenciados por `user_product_id`, com estoque e preço calculados pelo próprio ML. Entidade própria (`kits_virtuais`/`kits_virtuais_componentes`), fora do pipeline de produto por construção — o kit não é `familias` nem `anuncios_externos`. Preço sempre por desconto automático (nunca reprecificado pelo app); título por template (o ML expande sozinho) e descrição por IA uma vez no preview; foto própria obrigatória, subida ao ML no upload do diálogo (não no publicar); margem exibida como estimativa, nunca bloqueante nem um `0%`/`—` silencioso; imposto por origem sobre a parcela rateada de cada componente; "Refazer kit" encerra e recria (composição imutável no ML); componente de kit publicado bloqueado de remoção/republicação (guard de banco + guard de app em `remover-publicado`); kit aparece na tela Publicados (badge, preço/estoque ao vivo via `/sale_price` e `/user-products/{id}/stock`, sem Pausar/Reativar na v1) e nas vendas (badge, `ml_vendas.kit_item_id`, faturamento intocado). Elegibilidade medida nas duas contas reais (2026-09-06, só leitura): Avil 79 de 147 anúncios no modelo certo (138 UPs elegíveis), DSA 24 de 24 (14 elegíveis) — decide o item ser user product nativo, sem `variations[]`. Sonda de escrita (`POST /items/kits` inválido) devolveu `400` de validação nas duas contas, não `403` de certificação — endpoint autorizado, nenhum kit criado de verdade. Suíte da branch: 494 arquivos/4835 testes verdes. **Feito em 2026-09-06:** migration `20260906140450_kits_virtuais_schema.sql` aplicada, guards conferidos contra Postgres real (trigger de 2..6, `quantidade` 1..10, unicidade por kit), 24 Edge Functions deployadas (5 novas em v1 + 19 afetadas pelos `_shared`, dependentes mapeados por `deno info`) e `database.types.ts` regerado pela CLI. **Ainda aberto:** confirmar o parsing de `/sale_price` contra um kit real, o smoke test do guard de app de `remover-publicado` com kit publicado, e as três incógnitas de tarifa da Decisão 15 (só a primeira venda resolve). Ver [ADR-0154](decisions/0154-kits-virtuais-mercado-livre.md) e [Spike 036](spikes/036-kits-virtuais-mercado-livre.md).
