...
describe('Revisao — badge "kits aguardando" (ADR-0151 D-2)', () => {
  it('aparece SEM expandir a família quando há kits pronto e sem ml_item_id', () => {
    useKitsDoProdutoMock.mockReturnValue({
      data: [
        { familiaId: 'k1', codigoPai: '1001', multiplicador: 2, status: 'pronto', mlPermalink: null, mlItemId: null },
        { familiaId: 'k2', codigoPai: '1001', multiplicador: 3, status: 'pronto', mlPermalink: null, mlItemId: null },
      ],
    });
    renderRevisao();
    const badge = screen.getByText(/2 kits aguardando a publicação deste produto/);
  ...
  it('singular quando só 1 kit aguardando', () => {
    useKitsDoProdutoMock.mockReturnValue({
      data: [{ familiaId: 'k1', codigoPai: '1001', multiplicador: 2, status: 'pronto', mlPermalink: null, mlItemId: null }],
    });
    renderRevisao();
    expect(screen.getByText(/1 kit aguardando a publicação deste produto/)).toBeInTheDocument();
  });

  it('some quando o kit já foi publicado (tem ml_item_id)', () => {
    useKitsDoProdutoMock.mockReturnValue({
      data: [{ familiaId: 'k1', codigoPai: '1001', multiplicador: 2, status: 'publicado', mlPermalink: 'https://x', mlItemId: 'MLB1' }],
    });
    renderRevisao();
    expect(screen.queryByText(/aguardando a publicação/)).not.toBeInTheDocument();
  });

  it('não aparece quando não há kits vinculados', () => {
