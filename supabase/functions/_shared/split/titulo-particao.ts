...
import { garantirCorTitulo, garantirMetragemTitulo, garantirTipoProdutoTitulo, removerMarketingNaoGrounded } from '../ai/titulo.ts';
...
export async function gerarTituloParticao(opts: OpcoesTituloParticao): Promise<string> {
  try {
    ...
    const titulo = garantirMetragemTitulo(garantirTipoProdutoTitulo(removerMarketingNaoGrounded(out.titulo, opts.nome, opts.descricao_detalhado ?? ''), out.tipo_produto_busca), opts.nome); // ≤60, metragem preservada, tipo de produto garantido (ADR-0054), sem marketing não-grounded (lote #28)
