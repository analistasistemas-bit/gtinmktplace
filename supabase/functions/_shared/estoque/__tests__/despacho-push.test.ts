...
function coletor() {
  const jobs: Array<{ codigo_pai: string; canal_origem: string | null; reativar?: boolean }> = [];
  return {
    jobs,
    enfileirar: (job: { org_id: string; codigo_pai: string; canal_origem: string | null; reativar?: boolean }) => {
      jobs.push({ codigo_pai: job.codigo_pai, canal_origem: job.canal_origem, reativar: job.reativar });
...
describe('despacharPushPendente — a intenção de reposição chega no job', () => {
  const mov = (over: Partial<MovimentoPendente>): MovimentoPendente => ({
    id: 'm1', codigoPai: 'P1', canalOrigem: null, reposicao: false, ...over,
  });
