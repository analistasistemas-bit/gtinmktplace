-- Fix round 1 da Task 13 — C1 (CRÍTICO, produção): a migration anterior
-- (20260826061358_estoque_resumo_fiscal.sql) foi escrita a partir da versão ERRADA da RPC
-- (20260814181410, antes de P2.1) e não da última aplicada (20260814184715_estoque_p2_nomes_busca),
-- então o `create or replace` apagou `v.nome`/`array_agg(... ) as nomes`/`'nomes', p.nomes` —
-- a busca por nome de variação na tela Estoque quebrou em produção, para toda org, desde o push
-- anterior. Esta migration restaura `nomes` (base = P2.1) e MANTÉM os campos fiscais da task
-- anterior, mais os campos que o filtro fiscalPendente (I1) precisa pra espelhar o gate:
-- `cest`, `fci`, `tributacao_icms_regime` (unidade/origem já existiam no resumo).
